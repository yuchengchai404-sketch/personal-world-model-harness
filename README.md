...
> ⚠️ **运行边界：**本仓库包含可实际执行的 Markdown-native Harness、Runtime 指令、模板与脱敏案例，但不是一键运行的独立应用。它需要能够读取工作区指令并操作 Markdown 文件的兼容 Agent Runtime；作者的私人 Vault、实际工作区配置及平台 Runtime 不包含在内。
